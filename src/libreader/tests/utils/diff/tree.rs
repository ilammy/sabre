...
/// Compute the difference between two forests of trees of comparable elements.
...
/// Compute the difference between two forests of trees using the provided comparator.
pub fn diff_with<'a, T, F>(lhs: &'a [T], rhs: &'a [T], equal: F) -> Vec<Diff<'a, T>>
    where T: TreeNode, F: Fn(&T, &T) -> bool
{
    // We need this weird repackaging in order to be able to recurse.
    let lhs = lhs.iter().collect::<Vec<_>>();
    let rhs = rhs.iter().collect::<Vec<_>>();
    return child_diff(&lhs, &rhs, &equal);
}

/// Actually compute a diff between forest slices.
fn child_diff<'a, T, F>(lhs: &[&'a T], rhs: &[&'a T], equal: &F) -> Vec<Diff<'a, T>>
    where T: TreeNode, F: Fn(&T, &T) -> bool
{
    sequence::diff_with(lhs, rhs, |lhs, rhs| equal(lhs, rhs))
        .iter()
        .map(|diff| match *diff {
            ...
            sequence::Diff::Equal(lhs, rhs) => {
                self::Diff::Equal(*lhs, *rhs, child_diff(lhs.children().as_slice(),
                                                         rhs.children().as_slice(), equal))
            }
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn root_not_equal() {
        let a = vec![Tree::new(1, vec![])];
        let b = vec![Tree::new(2, vec![])];
    ...
    fn root_not_equal_children_equal() {
        let a = vec![Tree::new(1, vec![Tree::new(2, vec![]), Tree::new(3, vec![])])];
        let b = vec![Tree::new(2, vec![Tree::new(2, vec![]), Tree::new(3, vec![])])];
    ...
    fn root_equal_children_equal() {
        let a = vec![Tree::new(1, vec![Tree::new(2, vec![]), Tree::new(3, vec![])])];
        let b = vec![Tree::new(1, vec![Tree::new(2, vec![]), Tree::new(3, vec![])])];
    ...
    #[test]
    fn children_diff_1() {
        let a = vec![Tree::new(1, vec![Tree::new(2, vec![])])];
        let b = vec![Tree::new(1, vec![Tree::new(2, vec![]), Tree::new(3, vec![])])];
    ...
    fn children_diff_2() {
        let a = vec![Tree::new(1, vec![Tree::new(2, vec![]), Tree::new(3, vec![])])];
        let b = vec![Tree::new(1, vec![Tree::new(2, vec![])])];
    ...
    fn children_diff_3() {
        let a = vec![Tree::new(1, vec![Tree::new(2, vec![])])];
        let b = vec![Tree::new(1, vec![Tree::new(3, vec![])])];
            ...
            Diff::Equal(&a[0], &b[0], vec![
                Diff::Equal(&a[0][0], &b[0][0], vec![
                    Diff::Equal(&a[0][0][0], &b[0][0][0], vec![
                        Diff::Equal(&a[0][0][0][0], &b[0][0][0][0], vec![]),
                        Diff::Right(&b[0][0][0][1]),
                    ]),
                    Diff::Equal(&a[0][0][1], &b[0][0][1], vec![
                        Diff::Left (&a[0][0][1][0]),
                        Diff::Equal(&a[0][0][1][1], &b[0][0][1][0], vec![]),
                    ]),
                    Diff::Equal(&a[0][0][2], &b[0][0][2], vec![
                        Diff::Equal(&a[0][0][2][0], &b[0][0][2][0], vec![
                            Diff::Right(&b[0][0][2][0][0]),
                            Diff::Right(&b[0][0][2][0][1]),
                        ]),
                        Diff::Equal(&a[0][0][2][1], &b[0][0][2][1], vec![]),
                    ]),
                    Diff::Equal(&a[0][0][3], &b[0][0][3], vec![]),
                ]),
                Diff::Right(&b[0][1]),
                Diff::Equal(&a[0][1], &b[0][2], vec![
                    Diff::Equal(&a[0][1][0], &b[0][2][0], vec![
                        Diff::Left (&a[0][1][0][0]),
